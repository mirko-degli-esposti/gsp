...
> **Fonte non citata.** Questi cinque valori circolano nelle note di progetto
> senza riferimento. Vanno verificati sulla pubblicazione ISTAT prima di
> comparire in una figura, o tolti: una tacca di riferimento sbagliata sposta
> la lettura di ogni confronto.
...
1. **`CRONI` fra le opzionali?** +53% di pool e +50% di `n_eff` su tutte le
   variabili, al prezzo della copertura delle quattro a rotazione (§13.1).
