...
enrich.py               -> + sezione, area, paese (geografico),
   usa gsp.opendata         eta_anni, indirizzo, lon/lat      anello 3
                                                   |
assign_nucleo.py        -> id_nucleo, ruolo        |  anello 4
   usa gsp.nucleo          in FILE SEPARATO        |    (§16)
```

L'anello 4 **non tocca la popolazione**: scrive `data/nuclei/nuclei_{comune}.csv`
con `uid, id_nucleo, ruolo`, che si unisce per `uid`. La separazione e'
deliberata (§16.1) — il ruolo si deriva a valle, `|X|` resta invariato e non
serve alcun rifit.
...
- **`donor_id` non si ricostruisce più dalla firma.** Dal 2/8/2026 la
  pipeline lo scrive direttamente nel file di popolazione, quindi
  l'identificatore del rispondente è **quello vero** e le collisioni fra
  21-uple indistinguibili non entrano più nel conto. Tutto ciò che segue
  descrive la ricostruzione dalla firma, che resta la sola via quando si
  legge una popolazione generata prima di quella data.

  > **Verifica che vale più dell'argomento teorico.** Sostituendo la firma
  > ricostruita con l'identificatore vero, `n_eff` **sull'universo della
  > variabile non cambia**: su `PUNTIFI10`, Bologna 2.845 → 2.851, Modena
  > 3.220 → 3.227, Castenaso 2.770 → 2.774. Le differenze di qualche unità
  > sono i donatori che la firma faceva collidere. Sull'**intera
  > popolazione** invece Modena passa da 862 a 3.741, **un fattore 4,3**.
  >
  > È la quarta prova dello stesso punto, e la più netta: `n_eff` per
  > universo è invariante rispetto a **come si identifica il donatore**,
  > quello sulla popolazione intera no. La tabella di §13.3 resta valida
  > così com'è, confermata da due strade indipendenti.
...
## 16. L'anello 4 — nuclei familiari

*Sintesi per chi legge i file. La ricerca che c'e' dietro sta in
`note/nota_nucleo_familiare_v3.md`; qui c'e' cosa contengono i file, con
quali limiti, e come si uniscono alla popolazione.*

### 16.1 I file, e perche' sono separati
...
**1,91 milioni di individui in 888.000 nuclei**, undici comuni. La
popolazione non e' stata toccata: le due colonne stanno in file a parte e si
uniscono con un join su `uid`.

> **La separazione e' una decisione misurata, non una convenzione.** Il ruolo
> familiare si deriva **a valle**, con `(Relpar, Ncomp)` estratti come coppia
> e condizionati su `zona × eta × cittadinanza × sesso`. Conseguenza: `|X|`
> invariato, nessun rifit, nessun nuovo zero strutturale nel supporto. Se il
> ruolo fosse entrato nell'anello 1 avrebbe moltiplicato il supporto per sei.

`id_nucleo` ha la forma `340270000993-000041` e **contiene la sezione**:
quindi un nucleo a cavallo di due sezioni si vedrebbe a occhio. Non esistono
— verificato su tutti gli undici comuni.

Nel bundle del viewer l'id per esteso **non viene esportato**: la sezione e'
gia' nella riga, quindi basta il progressivo come `int16`. Sono 0,17 MB
invece di 4.

### 16.2 I ruoli, e la firma
...
La **firma** e' la stringa dei ruoli in ordine gerarchico — `RPFF`, non
`FFPR`. Il 96% dei nuclei ha ampiezza ≤ 4, e li' una o due firme coprono il
90%: `RPFF` da sola vale lo 0,91 dei nuclei di quattro componenti.

### 16.3 Chi non ha nucleo

**L'1,1–2,3% degli individui ha `id_nucleo` vuoto.** Non e' un errore: e'
popolazione in **convivenza anagrafica** — case di riposo, studentati,
caserme — dedotta dal residuo dei vincoli censuari, quando il residuo e'
troppo grande per essere troncamento della classe aperta `PF8`.

> **Chiude un punto aperto di §13.4.** L'AVQ **esclude gli istituti di
> convivenza**, mentre la popolazione sintetica li include: quegli individui
> ricevono donatori da un universo che non li contiene, e l'entita'
> dell'effetto era dichiarata **aperta**. Ora sono identificabili — sono
> esattamente quelli con `id_nucleo` vuoto — e l'effetto e' misurabile.
> L'anello 4 rende quantificabile un limite dell'anello 2.

### 16.4 Il vincolo di ampiezza, e il suo attrito

`gsp.nucleo.vincoli_da_sezione` ricava i nuclei per ampiezza da `PF3`–`PF8`
del censimento permanente 2023, **per sezione**. `PF3` = 1 componente,
`PF4` = 2, fino a `PF8` = **«6 e oltre»**, che e' una classe aperta.

Il totale coincide alla cifra: 85.249 famiglie su Modena, sia nel censimento
sia nella popolazione. La **distribuzione no** (**misurato**):

| ampiezza | sintetico | censimento | scarto |
|---|---|---|---|
| 1 | 35.577 | 35.300 | **+277** |
| 2 | 23.044 | 23.234 | −190 |
| 3 | 13.336 | 13.230 | +106 |
...
Meno dello 0,8% ovunque, ma non zero — e **lo scarto e' il contenuto
informativo**. Su 2.118 sezioni di Modena solo **994 tornano esatte**; le
altre 1.124 richiedono un aggiustamento, di tre tipi distinti:

```
sovrastima   419   il censimento vuole piu' famiglie di quante la
                   popolazione della sezione ne consenta
troncamento  366   la classe aperta PF8 conteneva nuclei > 6
convivenza   339   residuo troppo grande per essere troncamento
esatto       994
```

Il verso e' leggibile: piu' unipersonali e meno nuclei grandi di quanti il
censimento ne dichiari, coerente col troncamento della classe aperta.

> E' l'analogo per l'anello 4 dello z-score di §14.3: non «il fit ha
> rispettato il vincolo», ma **quanto attrito c'e' stato nel rispettarlo**.

### 16.5 Cosa NON e' vincolato

**La composizione interna del nucleo.** Le configurazioni vengono da un
repertorio costruito sui microdati AVQ, che e' **emiliano-lombardo e non
comunale**: la coda oltre l'ampiezza 6 viene dai soli microdati di Parma,
fonte mista e dichiarata.

Quindi la tipologia familiare — coppia con figli, monogenitore,
unipersonale — **non ha riferimento censuario** e va dichiarata come
modello. E' la stessa distinzione che §3.4 fa per gli incroci del constraint
set: alcuni sono osservati, altri sono estrapolazione a massima entropia.

L'asimmetria e' **misurata**, non assunta:

| | struttura geografica | vincolo |
|---|---|---|
| ampiezza del nucleo | forte fino alla sezione | marginali di sezione (`PF3`–`PF8`) |
| composizione interna | debole sotto il quartiere | condizionamento demografico |

### 16.6 I limiti dichiarati

**Il 18–24% dei coniugati non e' in una coppia di coniugati.** Il numero
salta all'occhio, ma scomposto per ruolo dice l'opposto di quel che sembra
(**misurato**, Modena):

```
17,7%   coniugati con ruolo F, A, G, N   il ruolo stesso esclude la coppia
 8,8%   coniugati R spaiati
 4,6%   coniugati P spaiati
```

Tre quarti non sono un difetto di appaiamento: sono **figli sposati che
vivono coi genitori**, che in Italia esistono — il 13,9% dei coniugati ha
ruolo `F`. Chi ha ruolo `P` e' appaiato correttamente nel 95% dei casi.

**La causa del resto e' a monte**: il constraint set non impone che i
coniugati siano in numero pari dentro una sezione, quindi in molte sezioni
restano dispari e qualcuno resta necessariamente spaiato.

> **L'anello 4 rivela un'incoerenza gia' presente nell'anello 1, non la
> crea.** E' lo stesso schema delle 26 combinazioni impossibili di §14.2:
> l'ISTAT non pubblica quel vincolo, MaxEnt non lo impone, e il difetto
> diventa visibile solo quando un anello successivo prova a usare
> quell'informazione.

La struttura e' **costante su undici comuni** — coniugati fuori dalle coppie
fra 14,3% e 18,6%, spaiati `R` fra 5,9% e 9,4% — il che esclude che sia un
artefatto di assemblaggio. Bologna e' il minimo su tutte e tre le colonne,
coerente con una citta' universitaria e terziaria: meno figli sposati in
casa, piu' nuclei piccoli, appaiamento piu' facile perche' ci sono piu'
coniugati per sezione.

**Altri limiti:**

- **nessuna coppia convivente dello stesso sesso.** Le unioni civili ci sono
  (`P_STESSO_SESSO_UNITI = 0,004`, da ISTAT), ma due `celibe_nubile` dello
  stesso sesso non si accoppiano mai: **manca la fonte**;
- **il divario d'eta' fra partner e' convenzionale** (±15 anni): l'AVQ ha
  classi larghe 5–10 anni e non lo risolve. Mediana realizzata −1, p05 −14,
  p95 +14;
- **il divario generazionale** ha mediana 29–30 anni contro un target AVQ di
  33, con lo 0,23% fuori dai limiti convenzionali 20–40;
- **l'indirizzo e' per individuo**, non per famiglia: marito e moglie possono
  risultare a civici diversi. Nel bundle pubblico la questione **non si
  pone**, perche' le coordinate sono randomizzate dentro la sezione e nessuno
  puo' dedurne niente.
...
97,2–98,8%              assemblati senza alcun ripiego
1,1–2,3%                degli individui in convivenza, senza nucleo
```

Ampiezza media per comune, dal censimento: **Bologna 1,83** — la piu' bassa
delle undici — contro **Rimini 2,15** e **Castenaso 2,16**.
...
**v2.4 — 15/08/2026**
Nuova §16, l'anello 4: nuclei familiari in file separati che si uniscono per
`uid`, senza toccare la popolazione. Il vincolo di ampiezza e' `PF3`–`PF8`
per sezione e il totale coincide alla cifra, ma la distribuzione no — e lo
scarto e' il contenuto informativo, perche' solo 994 sezioni su 2.118
tornano esatte. Il 18–24% di «coniugati incoerenti» scomposto per ruolo: tre
quarti sono figli sposati conviventi, non difetti di appaiamento. Chiuso un
punto aperto di §13.4: gli individui in convivenza, che l'AVQ esclude dal suo
universo, sono ora identificabili.

**v2.3 — 15/08/2026**
`donor_id` arriva dalla pipeline invece di essere ricostruito dalla firma
AVQ: l'identificatore del rispondente e' quello vero e le collisioni fra
21-uple non entrano piu' nel conto. La sostituzione **conferma** §13.3
invece di cambiarla — `n_eff` per universo di variabile resta identico alla
terza cifra su tutti gli undici comuni, mentre quello sull'intera
popolazione si muove di un fattore 4,3. Quarta prova che la prima misura e'
stabile e la seconda no.
