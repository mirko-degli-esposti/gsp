# Registro delle fonti, pacchetto `gsp`, attributi derivati — v6

Aggiornata il 4 agosto 2026. Sostituisce la v5. Rispetto a quella: il
registro ha **trentadue fonti**, `gsp.individui` implementa i tre regimi
di disclosure, il bundle pubblico è autoprotettivo per costruzione, e
`gsp.istruzione` attribuisce il **titolo di studio dettagliato** — il
secondo attributo derivato dopo il nome, e il primo che richiede un
raccordo dichiarato invece che una semplice lista.
...
- Il **ramo straniero** dei nomi: per paese da liste aperte, dalla fonte
  fiorentina con un classificatore, o generato morfologicamente. Con
  `fallback` dichiarato il sistema è già usabile — uno straniero prende
  nome e cognome italiani e la traccia lo dice.
