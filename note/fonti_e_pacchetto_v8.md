# Registro delle fonti, pacchetto `gsp`, attributi derivati — v8

Aggiornata il 5 agosto 2026. Sostituisce la v7. Rispetto a quella:
**trentasette fonti**, un quarto attributo derivato — settore e posizione
professionale — e soprattutto un **criterio misurabile** per decidere
dove un attributo debba stare, che fino a ieri era un giudizio (§8).

Il criterio ha già cambiato una decisione presa: il livello K10C del
MaxEnt, che includeva il settore economico nello spazio degli stati,
condiziona su una variabile sola quando ne servirebbero tre — al prezzo
di trentasette milioni di stati e di una catena di Gibbs riducibile.
...
~/progetti/gsp/
  ...
  src/gsp/
    ...
    lavoro.py              settore e posizione professionale
    tvd.py                 il criterio: distanza in variazione totale
...
`archivio_zip` serve anche a `cens2011_caratt_attl`, che è letta da
`gsp.lavoro` senza mai essere decompressa.
...
### Caratteristiche dell'attività lavorativa (1)

**`cens2011_caratt_attl`** — censimento 2011, quattordici dimensioni,
94 MB di soli codici. È la fonte di `gsp.lavoro` (§9), e il registro la
usa per la coppia congiunta settore × posizione professionale: 21 sezioni
ATECO × 6 profili × 2 sessi, condizionata su comune per sei comuni su
undici.

Nove anomalie in scheda, tutte trovate facendo girare il codice. Le due
che si rifarebbero: **i codici «totale» non sono uniformi** fra dimensioni
— `ALL`, `99`, `0010`, `9`, `TOTAL` — e chi ne assume uno solo filtra via
tutto; e **l'incrocio settore × titolo di studio a livello comunale
esiste solo per l'agricoltura**, il che ha cambiato il progetto della
derivazione.
...
## 8. Il criterio — dove deve stare un attributo

Fino al 4 agosto la domanda «questa variabile va nel MaxEnt o si deriva a
valle?» si rispondeva a giudizio. Dal 5 c'è una misura, ed è cambiata
una decisione già presa.

### La domanda

Un attributo può stare in due posti. Nel **modello congiunto**, dove il
MaxEnt lo genera insieme a tutti gli altri rispettando i vincoli
simultaneamente. Oppure come **derivazione a valle**, assegnato dopo,
pescando da una distribuzione condizionata sugli attributi già presenti.

Il costo dell'errore non è simmetrico. Metterlo nel modello congiunto
moltiplica lo spazio degli stati, può introdurre zeri strutturali che
rendono riducibile la catena, e vincola il solver. Lasciarlo fuori costa
solo l'informazione che porta — se ne porta.

### La misura

Per ogni variabile candidata `X` e ogni condizionamento possibile `S`:

    d(S) = TVD( P(X | S), P(X) )

dove la **distanza in variazione totale** è la quota di massa da spostare
per trasformare una composizione nell'altra. Va da 0 a 1 e si interpreta
senza scala: «una scheda su cinque sarà sbagliata».

- `d` grande per un `S` **disponibile a valle** → si deriva;
- `d` grande solo per un `S` **non disponibile** — tipicamente la
  geografia fine — → deve stare nel modello congiunto;
- `d` piccolo ovunque → si assegna senza condizionamento.

### Il caso della coppia

Se due variabili sono entrambe derivabili, resta da decidere se
separatamente o insieme:

    TVD( P(X, Y | S), P(X | S) · P(Y | S) )

Piccola: separatamente, ciascuna col suo condizionamento migliore.
Grande: insieme, anche a costo di un condizionamento peggiore per
entrambe.

E qui c'è un criterio secondario che il numero da solo non dà:

> **A parità di distanza, si preserva la struttura che rende
> riconoscibile un errore.** Una TVD di 0,15 sulla congiunta produce
> individui palesemente assurdi — dirigenti in agricoltura; una di 0,20
> sul condizionamento produce individui plausibili con proporzioni un po'
> storte. Un dirigente agricoltore lo nota chiunque; che i laureati siano
> il 12% invece del 18% in un settore, no.

È la stessa ragione per cui l'hot-deck AVQ copia il vettore intero
invece di campionare variabile per variabile (§5).

### `gsp.tvd`

Il criterio è una libreria, non uno script: `tvd()`, `profilo()`,
`riassunto()`, `indipendenza()`, `partizione()`, `foglie_candidate()`.

**Non importa nulla di GSP**: non conosce i comuni, non legge il
registro, non sa cosa sia una popolazione sintetica. Sta in `src/gsp/`
per comodità di import, ma il giorno che serve allegarlo a un articolo si
stacca copiando un file.

### Il controllo senza il quale il criterio non vale

> **Una TVD calcolata su supporti diversi non è una misura.**

Su fonti censuarie reali questo errore si è presentato **cinque volte in
...
1. titoli di studio con **una sola sezione** pubblicata confrontati con
   ...
   che sembrava il segnale più forte ed era il nulla;
2. la classificazione ATECO mescola sezioni (`A`-`U`) e aggregati
   (`0011` totale industria, `0012` totale servizi): la composizione
   sommava a più di uno;
3. i profili professionali hanno una gerarchia **non dichiarata dal
   codebook** — la colonna che sembra il padre è l'ordinamento — e la
   somma dei nove codici presenti faceva il 140% del totale;
4. i due livelli di dettaglio **non coincidono** fra la tavola dei totali
   e quella degli incroci: escludere un aggregato perdeva il 10% degli
   occupati;
5. e la quinta sul lato del **condizionamento** invece che della
   variabile misurata: la dimensione dell'età ha 82 modalità che si
   sovrappongono — anni singoli, quinquennali, decennali, aperti — e
   misurare su tutte mescola composizioni incomparabili.

Per questo `tvd()` **rifiuta** di misurare quando i supporti condividono
meno della metà delle modalità, e `partizione()` esiste per il controllo
da fare **prima**. `foglie_candidate()` ricostruisce la gerarchia dai
conteggi quando il codebook non la dichiara: su Parma ritrova l'albero
esatto dei profili professionali.

> Il controllo di partizione non serve solo alla variabile misurata:
> serve anche a quelle su cui si condiziona.

---

## 9. `gsp.lavoro` — settore e posizione

### La decisione, presa con la misura

| dimensione | TVD sulla composizione ATECO |
|---|---|
...
L'istruzione domina, il sesso e l'età sono comparabili fra loro, il
comune conta molto ma in modo disuguale.

Il livello K10C condizionava sul **solo sesso**. La derivazione a valle
usa sesso e comune, non tocca il solver, e tiene la coppia congiunta:
migliore su tutti e tre gli assi — informazione, costo computazionale,
correttezza.

### La coppia si estrae insieme

`ateco` e `profilo` sono fortemente dipendenti: la distanza fra la
congiunta e il prodotto delle marginali è **0,138–0,166** su cinque
territori. Stabile, quindi struttura e non rumore.

### Quello che la fonte non consente

Sembrava di poter condizionare anche sul titolo di studio, che è la
variabile più informativa. **Non si può**: l'incrocio settore × titolo a
livello comunale esiste solo per l'agricoltura.

Resta una **riponderazione** opzionale, spenta di default, che riscala la
congiunta per `P(ateco | titolo)` nazionale. Corregge parecchio — sposta
0,09-0,33 secondo il titolo — ma ha tre limiti che la rendono una scelta
e non un miglioramento: la marginale esiste solo a livello nazionale,
copre 14 sezioni su 21, e non è validabile perché il confronto naturale
con la regionale non esiste.

È attiva **solo nei prodotti narrativi**, dove la plausibilità
individuale conta più della correttezza aggregata: una laureata in
agricoltura si nota, uno scostamento del 5% su una marginale no. È la
stessa distinzione dei tre regimi (§10).

### I limiti, misurati

**Il 21,4% degli occupati di Parma sta nelle sette sezioni che la
riponderazione non raggiunge** — energia, acqua, trasporti,
informazione, immobiliare, attività professionali, servizi alle imprese.
Una scheda su cinque, e sono proprio quelle dove il titolo conterebbe di
più.

**L'età non è condizionata**, e con TVD fino a 0,27 sulle classi estreme
i ventenni ricevono la distribuzione dei cinquantenni. Attenuato dalla
riponderazione ma non annullato.

**Cinque comuni su undici ripiegano sulla regione**, e il costo non è
uniforme: Ravenna dista 0,048 dalla sua regione, Bologna 0,202. Il
ripiego costa poco dove il comune è ordinario e molto dove è particolare
— cioè proprio dove servirebbe.

---

## 10. Privacy e disclosure
...
**Dove sta un attributo è una misura, non un giudizio.** Un attributo va
nel modello congiunto solo se la sua struttura — tipicamente geografica —
non è deducibile da ciò che c'è già. Il criterio è la distanza in
variazione totale fra la composizione condizionata e quella marginale, e
si calcola *prima* di costruire (§8). Deciderlo per abitudine ha prodotto
K10C: trentasette milioni di stati, catena riducibile, e il vincolo
condiziona sulla variabile meno informativa delle tre disponibili.

**Un controllo di coerenza verifica una relazione sola.** Se il difetto
sta su un asse che quel controllo non guarda, passa in silenzio — e
sembra confermato proprio dal controllo che lo ignora. In `gsp.lavoro` il
primo controllo tornava a zero mentre la congiunta perdeva il 10% degli
occupati: ne servivano due, su assi diversi, e nessuno dei due da solo
bastava.
...
- **`gsp.lavoro` non condiziona sull'età**: TVD fino a 0,27 sulle classi
  estreme, quindi i ventenni ricevono la distribuzione dei cinquantenni.
  La correzione è tre bin — giovani, centrali, anziani — e va verificato
  che la tavola pubblichi l'incrocio a quel livello.
- **La riponderazione per titolo è costruita e spenta**: sposta 0,09-0,33
  secondo il titolo, ma usa una marginale nazionale su una congiunta
  comunale e non è validabile. Da decidere se tenerla anche fuori dai
  prodotti narrativi.
- **`c9_sex_posizione_prof.csv`** è costruito a ogni rigenerazione e mai
  letto da nessuno — l'unico riferimento in tutto il codice è
  `fetch_comune.py`, che lo scarica. Ora è anche superato da `gsp.lavoro`,
  che ha sei profili invece di due dalla stessa fonte.
- **`--riempi-sha`** dentro `gsp.fonti`, accanto a `--verifica`: oggi è
  uno script a parte.
...
### 13.2 Etichette di blocco disallineate
...
**I codici «totale» non sono uniformi dentro la stessa tavola.** In
`DICA_CARATT_ATTL` sono `ALL` per occupazione e cittadinanza, `99` per
profilo e titolo, `0010` per ateco, `9` per sesso e regime, `TOTAL` per
durata. Chi ne assume uno solo filtra via tutto e ottiene zero righe, che
sembra un problema di dati e non di filtro.
