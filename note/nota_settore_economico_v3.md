...
**v3 — 5 agosto 2026** · censimento 2011, `DICA_CARATT_ATTL`
Sostituisce la v2: la §6 ora dice cosa la tavola consente davvero, che è
meno di quanto sembrasse, e la §9 registra la via che resta aperta.
...
## 6. Come si costruisce, e perché non come si vorrebbe

### La tavola non consente il condizionamento pieno

Sembrava che si potesse fare `P(ateco | istruzione, sesso, comune)`.
**Non si può.** L'incrocio fra settore e titolo di studio a livello
comunale esiste **solo per la sezione A, agricoltura** — verificato il
5/8/2026: con `titolo` specificato le sezioni disponibili nei sei comuni
sono `['A']`, con `titolo = totale` sono tutte e ventuno.

È la stessa forma dell'artefatto della §4: la tavola pubblica gli incroci
profondi solo su un sottoinsieme, e chi non conta i supporti crede di
avere un dato che non c'è.

### Le due possibilità reali

```
A)  P(ateco, profilo | sesso, comune)        21 sezioni × 7 profili
        congiunta · geografia comunale · NIENTE titolo

B)  P(ateco | titolo, sesso, IT/regione)     14 sezioni × 14 titoli
    P(profilo | titolo, sesso, ...)
        marginali separate · titolo sì · assume indipendenza
```

### La misura che decide

`ateco` e `profilo` sono **fortemente dipendenti**. Confrontando la
distribuzione congiunta osservata con il prodotto delle sue marginali, a
parità di sesso e territorio:
...
conta più che perfezionare le marginali.

### La forma finale

```
P(ateco, profilo | sesso, comune)      6 comuni: Parma, Modena, Bologna,
                                        Brescia, Reggio Emilia, Ravenna
P(ateco, profilo | sesso, regione)     Rimini, Ferrara, Forlì, Piacenza,
                                        Castenaso
```
...
età centrali). Per tutti gli altri settore e profilo sono
`non_applicabile` per costruzione, non mancanti — come il missing
strutturale delle AVQ.

### I sette profili

Una partizione vera, molto migliore del `dipendente/indipendente` di
`c9`: dipendenti · lavoratore in proprio · imprenditore e libero
professionista · socio di cooperativa · coadiuvante familiare ·
parasubordinato.
...
## 9. La via aperta: riponderare con il titolo

La scelta A perde il condizionamento sul titolo di studio, che vale
0,105-0,390. Si può recuperarlo **senza rinunciare alla congiunta**.

L'idea: derivare la coppia `(ateco, profilo)` condizionata su sesso e
comune, poi **riponderare** con la marginale `P(ateco | titolo)`, che è
disponibile a livello nazionale e regionale su quattordici sezioni.

È un raking a una dimensione sulla congiunta, e il progetto ha già tutto
per farlo — è la stessa operazione dell'IPF dentro le zone.

**Il guadagno atteso**: entrambe le informazioni invece di una. La coppia
resta coerente perché la riponderazione non spezza la congiunta, e le
proporzioni per titolo si avvicinano a quelle vere.

**Il costo**: una funzione in più, e il rischio di introdurre un bias se
la marginale nazionale non fosse rappresentativa del comune.

**Il criterio per tenerla**: si costruisce, si misura quanto sposta la
distribuzione rispetto alla versione semplice, e si tiene solo se sposta
abbastanza da giustificarsi. Se lo spostamento fosse sotto 0,05, la
complessità non vale.

Da fare **dopo** la versione semplice, non insieme: la versione semplice
è già molto meglio di quello che c'è adesso, che è nulla.

---

## 10. E le altre variabili della stessa tavola
...
**`PROFILO_PROF` rende superfluo `c9`**: sette modalità utilizzabili
negli incroci profondi — dipendenti, lavoratore in proprio, imprenditore
e libero professionista, socio di cooperativa, coadiuvante familiare,
parasubordinato — contro le due di `c9`, dalla stessa fonte. (Le 47
nominali della codelist includono aggregati sovrapposti che negli
incroci non compaiono.)
