...
falsificate (§6), e una ritrattazione di metodo (§5.3).
...
| `Sesso` | 1 = M · 2 = F |
| `ETA` | anni compiuti |
| `Cittad` | codice paese ISTAT, **151 modalità**, 100 = Italia (82,03%) |
| `Ncomp` | componenti del nucleo |
| `Relpar` | relazione con la persona di riferimento |
...
### 2.3 `Relpar` — codifica *inferita*, non letta

| codice | n | lettura | base |
|---|---:|---|---|
| 1 | 96.995 | persona di riferimento | conteggio = famiglie attese per ogni `Ncomp`, scarto ~0,2% |
| 3 | 49.963 | figlio | mai con `Ncomp=1`, picco a `Ncomp=4` |
| 2 | 36.348 | partner | ≤ 1 per famiglia ovunque |
| 11 | 14.725 | **coabitante** | vedi sotto |
| 9, 4, 6, 8, 5, 7, 10 | 1.280 → 19 | altri parenti | — |

`Relpar=11` **non** può essere un secondo codice di partner: a `Ncomp=4`
darebbe 7.812 + 3.070 = 10.882 partner in 10.096 famiglie. Il profilo lo
qualifica come coabitazione: 14.724 persone (7,4%), età mediana **38**,
stranieri al **51,7%** contro il 17,4% della popolazione — fattore tre.
Non studenti: lavoratori migranti, con una coda universitaria.

> **La codifica è inferita dai conteggi, non letta su un codebook.** Va
> confermata con l'ufficio statistica del Comune prima che entri in un
> articolo.

**Conseguenza sostanziale**: la «famiglia anagrafica» a Parma non è un
oggetto omogeneo. Un nucleo di coinquilini e una famiglia coniugale hanno
regole di composizione incompatibili — nel secondo c'è al più un partner,
i divari d'età genitore-figlio stanno in una finestra; nel primo niente di
tutto ciò vale. L'assemblaggio richiede una **tipologia del nucleo** a
monte.
...
| **M3′** | è la coabitazione a guidarla? | M3 senza `Relpar=11` |
...
### 4.1 M0 — la coppia va estratta insieme
...
### 4.2 M1/M2 — ordine dei condizionanti
...
(0,018) ma **non** sul ruolo (0,08–0,086), e a posteriori è ovvio: la
persona di riferimento è più spesso maschio, il partner più spesso
femmina.
...
### 4.3 M3 — residuo geografico a demografia fissata
...
| `relpar` (senza `r11`) | 0,0382 | 0,0127 | **+0,0255** | 172 / 8 |
...
La riga per persona riproduce la v1 (1,39 → 1,40, M4a e M4b identici alla
quarta cifra): il codice della v2 non ha alterato nient'altro.
...
## 5. Cosa se ne conclude

### 5.1 Il ruolo si deriva a valle — anello 1 non si tocca
...
### 5.2 L'asimmetria ampiezza / ruolo è il risultato di disegno
...
### 5.3 Ritrattazione — la correlazione TVD ~ ampiezza
...
Sostituito dal netto per terzile di ampiezza (§4.4), che è il test
...
## 6. Le due previsioni falsificate

Registrate nel docstring degli script **prima** di far girare la misura.

**(a) M3′ — la coabitazione guida il residuo geografico.** Attesa: il
netto crolla escludendo `Relpar=11`, perché la coabitazione migrante è
...
Spiegazione post-hoc, non verificata: la concentrazione geografica della
coabitazione è quasi interamente mediata dalla cittadinanza, che è già in
C4. Verificabile rifacendo M3 su `relpar` senza `citt` fra i
condizionanti.
...
**(2) Conferma della codifica di `Relpar`** presso l'ufficio statistica
del Comune di Parma (§2.3).
...
## 8. Per il paper sul criterio TVD
