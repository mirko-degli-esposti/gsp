...
## Draft v0.1 — §IV.1–IV.3 (19 August 2026)
...
function of the tagged bundle. The one exception, declared rather than
solved, is the optional cartographic base layer, the only external service
the page can contact **[n]** §7.3.
...
The site is Cloudflare Pages, deployed from a folder:
...
why `build_bundle.py` exists and reports per-municipality status instead of
stopping at the first failure **[n]** §8, §15. GitHub Pages was rejected for
a reason that no longer applies (it requires a paid plan on private
repositories, and the repository was private while this report was being
prepared); the folder-based deploy remains because it leaves nothing to
clean and can actually be switched off. The page carries `noindex,
noarchive`, with the stated understanding that switching the site off does
not withdraw what has already been downloaded.
...
1. **[v]** Recommended citation format for a view (URL + version + hash):
   one worked example in the front matter.
2. **[v]** The cartographic base layer: either the Protomaps `.pmtiles`
   extract served locally (removing the last external dependency) or a
   sentence declaring it optional and off by default. Decide for v1.0.
3. **[v]** Re-measure the §7.2 cost figures (init, first query, lazy loads)
...
4. Screenshots for the report: three figures (marginals with three markers;
   trust with two bands; map in quota mode) taken from the deployed
   `report-v1.0` site at a fixed URL each, so the captions can carry the
   very URL they show.
